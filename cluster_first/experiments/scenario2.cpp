...
int main(int argc, char** argv) {
    ...
    if (argc != 2) {
        std::cerr << "Usage: scenario2 [datasets.txt]\n";
        return 1;
    }
    ...
    const std::filesystem::path combined_csv = scenario_dir / "sc_2_combined.csv";
    ...
    for (const auto& dataset : datasets) {
        ...
        const std::filesystem::path ds_csv = scenario_dir / ("sc_2_ds_" + std::to_string(dataset.index) + ".csv");
        ...
        ds_out << utils::csv_escape("scenario2") << ","
        ...
        combined_out << "scenario2," << dataset_name << "," << dataset_total << "," << dataset_time << "\n";
    }
