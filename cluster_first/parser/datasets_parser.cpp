...
std::vector<mcvrp::types::Coordinates> parse_coord_list(const std::string& s) {
...
std::vector<int> parse_weights_list(const std::string& s) {
    std::vector<int> out;
    std::string cur;
    std::istringstream iss(s);
    while (std::getline(iss, cur, ',')) {
...
std::vector<std::pair<std::string, mcvrp::types::GraphInput>> parse_datasets_txt(const std::string& filepath) {
    ...
    std::vector<std::pair<std::string, mcvrp::types::GraphInput>> datasets;

    std::string line;
    std::string current_name;
    std::vector<mcvrp::types::Coordinates> vehicle_coords;
    std::vector<mcvrp::types::Coordinates> target_coords;
    ...
    auto flush_dataset = [&]() {
        ...
        // Create a depot from the first vehicle coordinate if available, else (0,0)
        mcvrp::types::Node depot;
        depot.id = 0;
        depot.demand = 0;
        if (!vehicle_coords.empty()) {
            depot.coordinates = vehicle_coords.front();
        }
        g.nodes.push_back(depot);

        // Targets: ids start at 1
        for (std::size_t i = 0; i < target_coords.size(); ++i) {
            mcvrp::types::Node n;
            n.id = static_cast<int>(i) + 1;
            n.coordinates = target_coords[i];
            if (i < weights.size()) n.demand = weights[i];
            else n.demand = 0;
            g.nodes.push_back(n);
        }

        // Ensure even number of non-depot nodes by padding with a dummy node
        // located at the depot (id = next available). This mirrors the
        // preprocessing done in the original Python scripts so matching runs
        // on an even-sized vertex set (excluding depot).
        const std::size_t non_depot = (g.nodes.size() >= 1) ? (g.nodes.size() - 1) : 0;
        if ((non_depot % 2) == 1) {
            mcvrp::types::Node dummy;
            dummy.id = static_cast<int>(g.nodes.size());
            dummy.demand = 0;
            dummy.coordinates = depot.coordinates;
        ...
        for (std::size_t i = 0; i < g.nodes.size(); ++i) {
            for (std::size_t j = i + 1; j < g.nodes.size(); ++j) {
                const int w = mcvrp::utils::euclidean_distance_2dp_scaled(g.nodes[i].coordinates, g.nodes[j].coordinates);
                g.edges.push_back({g.nodes[i].id, g.nodes[j].id, w});
            }
        }

        datasets.emplace_back(current_name.empty() ? "dataset" : current_name, std::move(g));

        // reset
        current_name.clear();
        vehicle_coords.clear();
        target_coords.clear();
        weights.clear();
    };
    ...
    while (std::getline(in, line)) {
        ...
        if (t.rfind("Data set", 0) == 0) {
            // new dataset header
            // flush previous
            if (!current_name.empty() || !target_coords.empty()) {
                flush_dataset();
            }
            current_name = t;
            continue;
        ...
        // vehicle locations line
        if (t.find("Vehicle locations") != std::string::npos) {
            auto pos = t.find(":");
            if (pos != std::string::npos) {
                auto rest = t.substr(pos + 1);
                vehicle_coords = parse_coord_list(rest);
            }
            continue;
        }

        if (t.find("Target locations") != std::string::npos) {
            auto pos = t.find(":");
            if (pos != std::string::npos) {
                auto rest = t.substr(pos + 1);
                target_coords = parse_coord_list(rest);
            }
            continue;
        }

        if (t.find("Weights") != std::string::npos) {
            auto pos = t.find("=");
            if (pos != std::string::npos) {
                auto rest = t.substr(pos + 1);
                weights = parse_weights_list(rest);
            }
            // After weights we expect end of dataset; flush
            flush_dataset();
            continue;
        }
    }
    ...
    // final flush
    if (!target_coords.empty() || !current_name.empty()) {
        flush_dataset();
    }
